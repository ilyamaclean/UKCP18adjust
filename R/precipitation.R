...
#' @param observed a `spatialarray` of observed daily precipitation values
#' @param ukcp a `spatialarray` of ukcp daily precipitation values for the period
...
#' @export
rainzero_adj <- function(observed, ukcp) {
  a <- observed$arraydata
  v <- as.vector(a)
  v <- v[is.na(v) == F]
  sel <- which(v == 0)
...
#'
#' @param ukcp a `spatialarray` of UKCP18 daily rainfall data as returned by [cropresamplerain()]
...
#'
#' @export
rainapplyzero <- function(ukcp, rzero = 18.59483) {
  a <- ukcp$arraydata
  rd <- as.vector(a)
  s1 <- which(is.na(rd) == F)
  # Calculate order
